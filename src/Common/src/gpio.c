...
unsigned int  GPIOGetBitValue(uint32_t portNum, uint32_t bitPosi){
  switch (portNum){
        case PORT0:	 
	  return !!(LPC_GPIO0->DATA & (1<<bitPosi)); break;	
 	case PORT1:	
	  return !!(LPC_GPIO1->DATA & (1<<bitPosi)); break;	
	case PORT2:
	  return !!(LPC_GPIO2->DATA & (1<<bitPosi)); break;	 
	case PORT3:
	  return !!(LPC_GPIO3->DATA & (1<<bitPosi)); break;
	default:
          break;	  
  }
  return 1;
